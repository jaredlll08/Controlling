...
public class ControllingEvents {
    ...
    public static final Event<IEventHandler<KeyEntryListenersEvent, List<GuiEventListener>>> KEY_ENTRY_LISTENERS_EVENT = EventFactory.createArrayBacked(IEventHandler.class, listeners -> event -> listeners.length == 0 ? event.listeners() : Arrays.stream(listeners)
            .flatMap(handler -> handler.handle(event).stream())
            .toList());
    public static final Event<IEventHandler<KeyEntryMouseClickedEvent, Boolean>> KEY_ENTRY_MOUSE_CLICKED_EVENT = EventFactory.createArrayBacked(IEventHandler.class, listeners -> event -> listeners.length == 0 ? event.handled() : Arrays.stream(listeners)
            .anyMatch(handler -> handler.handle(event)));
    public static final Event<IEventHandler<KeyEntryMouseReleasedEvent, Boolean>> KEY_ENTRY_MOUSE_RELEASED_EVENT = EventFactory.createArrayBacked(IEventHandler.class, listeners -> event -> listeners.length == 0 ? event.handled() : Arrays.stream(listeners)
            .anyMatch(handler -> handler.handle(event)));
